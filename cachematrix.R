## The purpose of the program is to return the inverse of a matrix. If the 
## same matrix inverse is requested again, the program will return the cached
## inverse matrix instead of calculating the inverse matrix again. If the
## matrix is redefined, then it ignores the cached value and calculates the
## inverse of the newly defined matrix.

# makeCacheMatrix: Creates an object consisting of: 
#   'x'= An input matrix
#   'inv'= The input matrix inverse
#   'set()'= Sets x and inv=NULL in the parent environment
#   'get()'= Retrieves the input matrix
#   'setinv(inverse)= Sets inverse in parent directory
...
#cacheSolve: Checks if the 'x' object has been updated from the last time the \\
#cacheSolve function ran. If it has not, then it retrieves the cached inverse\\
